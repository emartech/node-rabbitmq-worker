...
describe('BaseWorker', function() {
  ...
  describe('.create', function() {
    it('returns a new instance', function() {
      expect(BaseWorker.create()).to.be.an.instanceof(BaseWorker);
    });
  });
  ...
  describe('#execute', function() {
    ...
    describe('creating logger', function() {
      context('not giving anything', function() {
        it('creates a new logger with class name as namespace', function() {
          worker = new BaseWorker();

          expect(worker.logger).to.be.an.instanceOf(logFactory.Logger);
          expect(worker.logger._namespace).to.be.eql('base-worker');
        });


        it('creates a new logger with extended class name as namespace', function() {
          class MyWorker extends BaseWorker {}
          worker = new MyWorker();

          expect(worker.logger._namespace).to.be.eql('my-worker');
        });
      });
      ...
      context('giving namespace', function() {
        it('creates a new logger', function() {
          ...
          expect(worker.logger._namespace).to.be.eql('logging-namespace');
        });
      });
    ...
    describe('logging', function() {
      ...
      context('when there is an error', function() {
        it('logs error', async function() {
          ...
          expect(logger.error).to.have.been.calledWith('failure', { message: 'Some error', foo: 'foo', bar: 'bar' });
        });
