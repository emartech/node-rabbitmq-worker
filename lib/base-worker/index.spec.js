...
describe('BaseWorker', function() {
  ...
  describe('#queueWorker', function() {
    ...
    it('creates new queue item to given worker', async function() {
      this.sandbox.stub(QueueManager.prototype, 'queueWorker');

      const worker = new BaseWorker('logging-namespace');
      await worker.queueWorker('MyOtherWorker', { foo: 'bar' });

      expect(QueueManager.prototype.queueWorker)
        .to.have.been.calledWithExactly('MyOtherWorker', { foo: 'bar' });
    });
